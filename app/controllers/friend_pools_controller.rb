class FriendPoolsController < AuthController
  ...
  def create
    ...
    query = "SELECT * FROM users WHERE user.id IN (SELECT id FROM (SELECT friend_id FROM friendships WHERE user_id = #{@game.player1id}) AS f1 INNER JOIN (SELECT friend_id FROM friendships WHERE user_id = #{@game.player2id}) AS f2 ON f1.friend_id = f2.friend_id)"
