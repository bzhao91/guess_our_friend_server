class GamesController < AuthController
  ...
  def set_rematch
    ...
    if @game.player1rematch == true && @game.player2rematch == true
      ...
      send_gcm_message(User.find_by_id(opponent).gcm_id, "Your friend #{@current_user.first_name} has accepted the rematch!", {state: 1, game_id: g.id, opponent_id: User.find_by_id(opponent).fb_id, opponent_first_name: User.find_by_id(opponent).first_name, opponent_last_name: User.find_by_id(opponent).last_name, message: "Successfully created rematch."}.to_json)
