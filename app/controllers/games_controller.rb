class GamesController < AuthController
  ...
  def show_all_games
    query_incoming = "SELECT * FROM (SELECT id as game_id, player2id FROM games WHERE player1id = #{@current_user.id}) AS g INNER JOIN users on g.player2id = users.id"
    results_incoming = User.find_by_sql(query_incoming)
    query_outgoing = "SELECT * FROM (SELECT id as game_id, player1id FROM games WHERE player2id = #{@current_user.id}) AS g INNER JOIN users on g.player1id = users.id"
