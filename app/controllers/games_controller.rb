class GamesController < AuthController
  ...
  def show_game_board
    ...
    query_incoming = "SELECT users.first_name, users.last_name FROM (SELECT * FROM friend_pools WHERE user_id = #{@opponent_id} AND game_id = #{@game.id}) AS pools INNER JOIN users ON user.id = pools.friend_id"
    incoming_list = User.find_by_sql(query_incoming)
    query_outgoing = "SELECT users.first_name, users.last_name, pools.grey FROM (SELECT * FROM friend_pools WHERE user_id = #{@current_user.id} AND game_id = #{@game.id} AS pools INNER JOIN users ON user.id = pools.friend_id"
