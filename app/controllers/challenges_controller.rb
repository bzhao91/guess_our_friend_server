class ChallengesController < ApplicationController
  ...
  def create
    ...
    challenge = @current_user.sending_challenges.create(challengee: challengee.id)
    if challenge.save
  ...
  def show_outgoing_challenges
    query = "SELECT * FROM ((SELECT id, challengee_id FROM challenges WHERE challenger_id = #{@current_user.id}) AS c INNER JOIN users on users.id = c.challengee_id)"
